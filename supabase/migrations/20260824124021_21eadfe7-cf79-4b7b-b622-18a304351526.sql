CREATE OR REPLACE FUNCTION public.rematricula_2027_admin_editar_valores(
  p_id_aluno bigint,
  p_percentual_desconto numeric,
  p_percentual_desconto_ext text,
  p_valor_com_desconto numeric,
  p_valor_com_desconto_ext text
)
...
BEGIN
  ...
  IF p_percentual_desconto IS NULL OR p_percentual_desconto < 0 OR p_percentual_desconto > 60
     OR (p_percentual_desconto::numeric % 5) <> 0 THEN
    RETURN QUERY SELECT false, 'Percentual inválido (0 a 60, de 5 em 5).'::text;
    RETURN;
  END IF;
...
REVOKE ALL ON FUNCTION public.rematricula_2027_admin_editar_valores(bigint, numeric, text, numeric, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.rematricula_2027_admin_editar_valores(bigint, numeric, text, numeric, text) TO authenticated;